	...
	<?php foreach ( $requests as $request ) : $status = Request_Type::get_status( $request->ID ); ?>
		<article class="dv-request-card">
			...
			<form class="dv-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
				...
				<p><label>وضعیت <select name="status"><?php foreach ( Request_Type::admin_statuses() as $candidate ) : ?><option value="<?php echo esc_attr( $candidate ); ?>" <?php selected( $status, $candidate ); ?>><?php echo esc_html( Request_Type::statuses()[ $candidate ] ); ?></option><?php endforeach; ?></select></label></p>
